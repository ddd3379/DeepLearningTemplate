...
class TrainClass():
    ...
    def train(self, model, optimizer, E, iteration, dataloader, valid=False, verbose=1):
        ...
        for i in range(iteration):
            ...
            train_losses = []
            for _, (data, target) in enumerate(dataloader['train']):
                # 入力を一次元に変換
            ...
            if valid:
                ...
                with torch.no_grad():
                    ...
                    for _, (data, target) in enumerate(dataloader['valid']):
                        # 入力を一次元に変換
                        data = data.view(-1, 28*28)
                        ...
                        corrects.append(pred.eq(target.view_as(pred)).sum().item() / torch.numel(pred))
                    ...
                    losses['valid_acc'].append(correct)

            # ----- verboseにより詳細の表示・非表示 -----
            if verbose != 0:
                outputText = 'epoch(%d) : loss(train)=%f' % (i+1, train_loss)
                if valid:
                    outputText += ', loss(valid)=%f, acc(valid)=%f' % (valid_loss, correct)
